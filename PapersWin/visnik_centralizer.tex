...
У даній статті будемо використовувати наступні означення:

\begin{defi}

$T_2$ - бінарне кореневе дерево,

$AutT_2$ - група автоморфізмів $T_2$,

$x*a$ -  дія автоморфізму $a$ на кінець $x$ дерева $T_2$,

$a\circ b$ - суперпозиція автоморфізів $a$ та $b$ дерева $T_2$
\end{defi}

Автоморфiзм $a\in AutT_2$ iндукує дiю на пiддеревах $T_2$. Назвемо $a$ скiнчено-становим,
якщо вiн iндукує скiнченну кiлькiсть дiй на пiддеревах.
\begin{defi}
$FAutT_2$ - група скінчено-станових автоморфізмів $T_2$

$Z_2$ - кільце цілих 2-адичних чисел,
\end{defi}

 Ребрам дерева $T_2$ можна приписати мiтки — 0,1 для лiвого та правого ребра, що йдуть униз. При цьому кожному нескiнченному шляху на деревi, що починається
з кореня, буде вiдповiдати нескiнченна послiдовнiсть нулiв та одиниць, яку можна зiставити
з цiлим 2-адичним числом. Пiсля цього автоморфiзми дерева $T_2$ можуть бути ототожненi з ізометріями кiльця цiлих 2-адичних чисел $Z_2$.
\begin{defi}
Ототожнімо автоморфізм $a$ дерева $T_2$ з функцією $f_a:Z_2\rightarrow Z_2$ наступним чином:
$$x*a = f_a(x)$$
\end{defi}

Наприклад, нижченаведений 2-становий автоморфiзм $\varepsilon$
можна визначити рекурентно:
$$\varepsilon = (id, \varepsilon) \circ \sigma$$
$$id = (id,id)$$
тут вказано, що автоморфiзм $\varepsilon$ дiє на лiвому пiддеревi тотожно, на правому самоподiбно,
а $\sigma$ переставляє цi пiддерева.
З iншого боку, автоморфiзм $\varepsilon$ може бути визначений як функцiя $f_{\varepsilon}(x)=x+1$,
i тому має назву “додавальна машина“ (adding machine).
\begin{defi}
Будемо казати, що автоморфізм $\chi_0$ є
0-розв'язком рівняння $a^{\chi_0}=b$, якщо $$0*\chi_0=0$$
\end{defi}


В роботі \cite{Z} автором було доведено наступну теорему:
...
Нехай $\varepsilon$ - двостановий автоморфізм adding machine, що задається співвідношеннями:
$$\varepsilon = (id, \varepsilon)\circ \sigma$$ 
$$ id = (id, id)$$ 



Нехай $\varepsilon$ - adding machine, тобто $x*\varepsilon = x+1$. Тоді має місце наступна лема:
...
Будемо казати, що $t_k$ належить k-му рівню в $P_{m,n}$, якщо отримано з $m$ за k кроків.
 ...
 \begin{lemma}\label{F_P_equal}
   Нехай  2-адичне квазіперіодичне число  $p$ дорівнює $\frac{m}{2n+1}$, де $ m\in \mathbb{Z}, n\in \mathbb{Z}^+\cup 0$. Тоді множини $P_{m,n}$ та  $F_p $ скінчені або нескінчені одночасно.
 \end{lemma}
 \begin{proo}
...
\begin{lemma}\label{F_finite}
  Множина $F_p$ скінчена тоді, і тільки тоді, коли $p$ - квазіперіодичне число.
\end{lemma}
\begin{proo}

  $\Rightarrow$ Для $2t+1$ та $2t$ число $t$ отримується відкиданням останьої цифри у двійковому запису, отже $F_p$ містить всі числа,  що отримуються з   $p$ відкиданням декількох останніх цифр. Якщо $p$ не квазіперіодичне, то маємо нескінчену кількість таких чисел, тому  $F_p$ не є скінченою.
...
Далі, нехай $p\in Z_2 \cap \mathbb{Q}\}$, тобто квазіперіодичне. За лемою \ref{F_finite}  множина $F_p$ - скінчена. З іншої сторони, мають місце рівності:
...
Оскільки $F_p$ - скінчена, то $\varepsilon^{p}$ - скінчено-становий автоморфізм, щ.т.д.
\end{proo}
