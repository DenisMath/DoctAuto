...
{\itshape In this work the centralizer of  spherical-transitive finite-state automorphisms investigated.\\
...
Статтю представив доктор фіз.-мат. наук Боднарчук Ю.В. %
...
$ x * a $ - action of the automorphisms $ a $ on the end $ x $ of the tree $ T_2 $,


$ a \circ b $ - superposition of automorphisms $ a $ and $ b $ of the tree $ T_2 $,


$ Z_2 $ - ring of integer 2-adic numbers


We say that an automorphism $ \chi_0 $ is 0 -~solution of the equation $ a ^ {\chi_0} = b $, if $ 0 * \chi_0 = 0 $
\end{defi}



We identify automorphism $ a $ tree $ T_2 $ with the function $ f_a: Z_2 \rightarrow Z_2 $ as follows:
$$ x * a = f_a (x) $$

In the paper \cite{Z} author has proved the following theorem:

 

\begin{thm}\label{SPTR_centralizer}
    Let $ x $ - spherical-transitive automorphism, then
  $$ C_{AutT_2}(x) = \{x^p | p \ in Z_2 \} $$
  \end{thm}

The aim of this work is to study the centraliser of spherical-transitive elements in $ FAutT_2 $, because there are no results such as theorem \ref {SPTR_centralizer} for $ FAutT_2 $ .



 \section{Centralizers of  spherical-transitive elements in $FAutT_2$}

Let $\varepsilon$ - adding machine, that is $x*\varepsilon = x+1$. Then there is the following lemma:
\begin{lemma}\label{nul_epsilon}
 For $p\in Z_2$ we have the following equation:
   $$0*\varepsilon^p=p$$
\end{lemma}
\begin{proo}
Since $t*\varepsilon^p = t+p$, then $0*\varepsilon^p = 0+p = p $.
\end{proo}

\begin{thm}\label{nul_action_all}
  Let $\chi_x$ - 0 -~solution of the equation  $\varepsilon^t = x$ relatively automorphism t. Then there is the equation:
   $$0*x^p=p*\chi_x$$
\end{thm}
\begin{proo}
Since  $\varepsilon^{\chi_x} = x$, then:
$$x^p = (\chi_x^{-1}\circ\varepsilon\circ\chi_x)^p=\chi_x^{-1}\circ\varepsilon^p\circ\chi_x$$

So by lemma \ref{nul_epsilon} and equity $ 0 * \chi_x = 0 $ we get:
...
We have the following lemma:
 ...
 \begin{lemma} Let $x$ -spherical-transitive automorphisms. Then
 $$0*C_{AutT_2}(x)=Z_2 $$
  \end{lemma}
  \begin{proo}
...
щ.т.д.
 ...
 \begin{lemma}\label{P_finite}
   Множина $P_{m,n} ( m\in \mathbb{Z}, n\in \mathbb{Z}^+\cup 0 )$ є скінченною.
 \end{lemma}
 \begin{proo}
...
тому множина $P_{m,n}$ є скінченною, щ.т.д.
 \end{proo}
...
\begin{proo}
  ...
  $\Rightarrow$ Для $2t+1$ та $2t$ число $t$ отримується відкиданням останьої цифри у двійковому запису, отже $F_p$ містить всі числа,  що отримуються з   $p$ відкиданням декількох останніх цифр. Якщо $p$ не квазіперіодичне, то маємо нескінчену кількість таких чисел, тому  $F_p$ не є скінченною.
...
Оскільки $F_p$ - скінчена, то $\varepsilon^{p}$ - скінченно-становий автоморфізм, щ.т.д.
\end{proo}
...
\end{proo}
 ...
 %  \begin{defi} Означимо функцію $h_x:C_{AutZ_2}(x)\rightarrow Z_2$, де $x$ - шарово-транзитивний скінченно-становий автоморфізм, наступним чином: $$h_x(u)=0*u$$
 % \end{defi}

 % Зауважимо, що згідно з означенням

 % \begin{thm} Скінченно-становий автоморфізм $x$ є шарово-транзитивним тоді, і тільки тоді, коли
 % $h_x:C_{AutT_2}(x)\rightarrow Z_2$ - бієкція.
 % \end{thm}

 
   \begin{thm} Нехай $x$ - шарово-транзитивний скінченно-становий автоморфізм. Тоді
